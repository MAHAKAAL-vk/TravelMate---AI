...
class BrowserAgent:
    def __init__(self):
        self.stored_table = None  # Used to keep table separate
...
class TableCreatorTool(Tool):
    ...
    def execute(self, input):
        ...
        for place in tourism_places:
            ...
            data = {
                ...
                "📝 Description": self.short_snippet(f"describe {place}"),
                "🕒 Timings": self.short_snippet(f"visiting timings of {place}"),
                "💰 Entry Fees": self.short_snippet(f"entry fee of {place}"),
                "🌤️ Best Time to Visit": self.short_snippet(f"best time to visit {place}"),
                "📅 Upcoming Holidays": self.extract_holidays(place),
                "📍 Google Map Link": f"[Google Map]({map_link})",
                "📖 Mentioned In": self.extract_sources_from_links(self.perform_search(place)),
            }
            table.append(data)
    ...
    def short_snippet(self, query):
        data = self.perform_search(query)
        return data.get("organic", [{}])[0].get("snippet", "Not found")

    def extract_holidays(self, place):
        today = datetime.now().strftime("%A, %d %B %Y")
        snippet = self.short_snippet(f"upcoming holidays near {place}")
        holidays = [f"Today: {today}"]
    ...
    def extract_sources_from_links(self, results):
        try:
            links = [res.get("link", "") for res in results.get("organic", [])]
            sources = []
            for link in links:
            ...
            return ", ".join(set(sources)) if sources else "Online Sources"
        except Exception as e:
    ...
    def perform_search(self, query):
        try:
            headers = {"X-API-KEY": SERPER_API_KEY, "Content-Type": "application/json"}
            response = requests.post("https://google.serper.dev/search", headers=headers, data=json.dumps({"q": query}))
            return response.json()
        except Exception as e:
